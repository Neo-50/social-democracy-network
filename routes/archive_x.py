...
# _PIC_RE = re.compile(r'https?://pic\.twitter\.com/\S+')
...
# 	images = []
# 	html_text = fetch_tweet_html(tweet_id) or ''
# 	if html_text:
# 		with open(f"debug_tweet_{tweet_id}.html", "w", encoding="utf-8") as f:
# 			f.write(html_text)
# 		pbs_in_page_plain = 'pbs.twimg.com' in html_text
# 		pbs_in_page_esc   = 'pbs\\/twimg\\.com' in html_text or 'pbs\\u002Etwimg\\u002Ecom' in html_text
# 		print(f"[DIAG] tweet page: len={len(html_text)} pbs_plain={pbs_in_page_plain} pbs_esc={pbs_in_page_esc}")
...
# def resolve_pic_shortlinks(text: str):
#     """Return list of final pbs URLs by following pic.twitter.com redirects."""
#     if not text:
#         return []
#     urls = _PIC_RE.findall(text)
#     out = []
#     for u in urls:
#         try:
...
#             final = r.url or ''
#             if 'pbs.twimg.com' in final:
#                 # normalize to name=orig
#                 parts = urllib.parse.urlsplit(final)
#                 q = urllib.parse.parse_qs(parts.query, keep_blank_values=True)
#                 q['name'] = ['orig']
#                 new_q = urllib.parse.urlencode({k: v[-1] for k, v in q.items()}, doseq=True)
#                 final = urllib.parse.urlunsplit((parts.scheme, parts.netloc, parts.path, new_q, parts.fragment))
#                 out.append(final)
#         except Exception:
#             continue
#     # de-dupe preserve order
